...
[X, targets] = ProcessDemos(demos, 2, demo_struct, process_options);
Data = X(1:end-1,:);
...
[X_grid, X_d] = plotStreamLines(Priors,Mu,Sigma, [-200 200 -200 200]);
hold on
...
lyap_options.xlims = [-200 200];
lyap_options.ylims = [-200 200];
...
lyapFun = @(x) (x-targets')'*(x-targets')/2;
lyapFun_d = @(x) (x-targets');
