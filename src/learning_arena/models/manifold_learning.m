...
%             obj.graph_options_ = [obj.graph_options_, varargin];
        ...
        % eigenvalues
        function [V,D,W] = eigensolve(obj, data, varargin)
            if nargin > 1; obj.set_data(data); end
            
            if ~obj.is_eigen_
                [obj.right_vec_, obj.eig_, obj.left_vec_] = obj.solve(varargin{:});
                obj.is_eigen_ = true;
            end
        ...
        function varargout = plot_eigenfun(obj, space, varargin)
            ...
            for i = 1 : length(space)
                ...
                varargout{i} = figure;
                title(['Eigenfunction ', num2str(space(i))]);
                subplot(1,2,1)
        ...
        % Plot embedding
        function fig = plot_embedding(obj, space)
            ...
            if length(space) == 2
                fig = scatter(U(:,space(1)),U(:,space(2)), 20, 'r','filled','MarkerEdgeColor',[0 0 0]);
            else
                fig = scatter3(U(:,space(1)),U(:,space(2)),U(:,space(3)), 20, 'r','filled','MarkerEdgeColor',[0 0 0]);
            end
            grid on;
        end
